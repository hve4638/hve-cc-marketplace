...
/**
 * Read `.git/HEAD` directly and extract a 7-char commit hash when the
 * repository is in detached HEAD state. Returns null when HEAD holds a
 * symbolic ref (branch), when .git is missing, or on any error.
 */
function readDetachedHeadShort(cwd?: string): string | null {
  try {
    ...
    if (!head || head.startsWith('ref:')) return null;
    // Raw commit hash on detached HEAD
